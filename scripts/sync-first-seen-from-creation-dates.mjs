...
// Mirrors src/lib/image-library.ts's virtual path prefix for extra-root images, so first-seen
// cache keys generated by this script line up with the ones the running app reads/writes.
...
const buildExtraRootRelativePrefix = (extraRootIndex) => {
  return `${EXTRA_ROOT_PATH_SEGMENT}/${extraRootIndex}`;
};

const directoryHasCharactersFolder = (directoryPath) => {
  try {
...
// Mirrors src/lib/image-library.ts's resolveExtraImageRoots: each configured entry is either an
// images root itself (it directly contains a "characters" folder) or a parent directory whose
// immediate subdirectories are each their own images root.
const resolveExtraImageRoots = () => {
  const rawValue = process.env.SD_EXTRA_IMAGES_ROOT?.trim();
...
const run = async () => {
  ...
  const extraRootPaths = resolveExtraImageRoots();
